...
function smsg(conn, m, store) {
    ...
    if (m.message) {
        ...
        // 1. Robust Unwrapping (handle multiple layers if necessary)
        while (m.mtype === 'viewOnceMessage' || m.mtype === 'viewOnceMessageV2' || m.mtype === 'ephemeralMessage') {
        ...
        // 2. ULTRA-ROBUST ID/TEXT EXTRACTION (Recursive Search)
        let extractedId = '';
        const searchForCommand = (obj) => {
            if (!obj || typeof obj !== 'object') return;
            // Check common ID keys first
            const keys = ['selectedId', 'selectedButtonId', 'selectedRowId', 'id'];
            ...
            // Fallback: check ANY string value for command pattern
            for (const key in obj) {
                if (typeof obj[key] === 'string' && (obj[key].startsWith('.') || obj[key].startsWith('!') || obj[key].startsWith('/'))) {
                    extractedId = obj[key];
                    return;
                }
                if (typeof obj[key] === 'object' && !extractedId) searchForCommand(obj[key]);
            }
        };

        searchForCommand(m.message);

        // Special handling for Interactive Responses (Native Flow)
        if (!extractedId && m.mtype === 'interactiveResponseMessage') {
            try {
                const params = JSON.parse(m.msg.nativeFlowResponseMessage?.paramsJson || '{}');
                if (params.id) extractedId = params.id;
            } catch (e) { }
        }

        // 3. Populate m.body and m.text with Priority
        m.body = extractedId || m.message.conversation || m.msg?.caption || m.msg?.text || m.msg?.contentText || m.msg?.selectedDisplayText || m.msg?.title || '';
        m.text = m.body;
