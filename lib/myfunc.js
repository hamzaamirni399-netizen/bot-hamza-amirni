...
function smsg(conn, m, store) {
    ...
    if (m.message) {
        ...
        if (m.mtype == 'viewOnceMessage' || m.mtype == 'viewOnceMessageV2') {
             const nestedType = getContentType(m.message[m.mtype].message);
             m.mtype = nestedType;
             m.msg = m.message[m.mtype].message[nestedType];
        }
        ...
        // --- DEEP SEARCH FOR ID (BUTTONS) ---
        if (!m.body || m.body === '') {
             if (m.msg?.nativeFlowResponseMessage?.paramsJson) {
                  try { m.body = JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id; } catch(e){}
             }
             if (!m.body && m.msg?.selectedButtonId) m.body = m.msg.selectedButtonId;
             if (!m.body && m.msg?.singleSelectReply?.selectedRowId) m.body = m.msg.singleSelectReply.selectedRowId;
             if (!m.body && m.msg?.selectedId) m.body = m.msg.selectedId;
        }
        
        // --- FALLBACK TO INTERACTIVE MESSAGE CONTENT ---
        if (!m.body && m.mtype === 'interactiveResponseMessage') {
             const flow = m.msg.nativeFlowResponseMessage;
             if (flow) {
                  try {
                       const params = JSON.parse(flow.paramsJson);
                       m.body = params.id || params.text;
                  } catch (e) {}
             }
        }
